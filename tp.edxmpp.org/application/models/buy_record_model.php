...
class buy_record_model extends CI_Model
{
    ...
    public function getPaymentHistoryPoint()
    {
        return $this->db->select('ph_id AS id, m_name AS name, m_photo AS photo, "鑽石" AS project, ph_price AS price, ph_status AS status, ph_date AS date')
            ...
            ->where('payment_history.m_id', $_SESSION['Mid'])
    ...
    {
        return $this->db->select('ph_id AS id, m_name AS name, m_photo AS photo, "購買課程" AS project, ph_price AS price, ph_status AS status, ph_date AS date')
            ->from('payment_history')
            ->where('payment_history.m_id', $_SESSION['Mid'])
            ->where('ph_project !=', 'point')
            ->join('member', 'payment_history.m_id = member.m_id', 'inner')
