...
$(document).ready(function () {
    ...
    buildHandOutGoldsRecordTable(date); //生成鑽石發放紀錄

    // $('.display_sel_history')
    //     .change(function () {
    //         if ($("input[name='reportManagementOption__control']:checked").val() == 'Issue') {
    //             buildHandOutGoldsRecordTable($("#date").val()); //生成鑽石發放紀錄
    //         }
    //         else {
    //             GoldsRecord($("#date").val()); //生成鑽石儲值紀錄
    //         }
    //     });
...
function operateFormatter(value, row, index) {
    ...
    return [
        '<a class="btn btn-outline-primary mb-2" onclick="cancelHandOutGolds(\'' + row['id'] + '\');" title="' + text + '" target="_blank">',
        text,
        '</a>',
    ].join('')
}
...
function buildHandOutGoldsRecordTable(date) { //生成金幣發放紀錄
    convey_data = {
        date: date,
    };
    ...
    $.ajax({
        type: "POST",
        url: "hand_outGolds/getHandOutGoldsRecord",
        data: convey_data,
        dataType: 'json',
...
function handOutGolds() { // 開啟發放金幣視窗
    $("#hand_outGolds_window").modal();
    $("#confirm").attr("onclick", "confirmHandOutGolds()");
}

function selectOptionForUserID() { // 選擇發放金幣類型
    ...
    if (type == "全體") {
        ...
        $("#userID").val("all_random");
        $("#userID").attr("disabled", "disabled");
        $("#quota_area").css("display", "block");
    }
}

function confirmHandOutGolds() { // 確定發放金幣
    if($("#userID").val() == "all_random"){ //選擇隨機發放比較特別
        var convey_data = {
            id: $("#userID").val(),
            quota: $("#quota").val(), //名額
            gold: $("#gold").val(),
    ...
    $.ajax({
        type: "POST",
        url: "hand_outGolds/handOutGolds",
        data: convey_data,
        dataType: 'json',
        success: function (res) {
            //輸出訊息
            ...
            if(res['status']){
                $('#hand_outGolds_window').modal('hide');
            }
...
function getHandOutGoldsRecordSpecific(id) { //取得金幣發放紀錄(指定人顯示)
    convey_data = {
        id
    };
    ...
    $.ajax({
        type: "POST",
        url: "hand_outGolds/getHandOutGoldsRecordSpecific",
        data: convey_data,
        dataType: 'json',
        success: function (res) {
            //輸出訊息
...
function cancelHandOutGolds(id) { //取消金幣發放
    var hint_text = "";

    $('#hint_text').html("");
    $('#hint_Title').text("取消金幣發放提示");
    ...
    $('#hint_confirm').unbind('click').bind('click', function (e) {
        ...
        $.ajax({
            type: "POST",
            url: "hand_outGolds/cancelHandOutGolds",
            data: convey_data,
            dataType: 'json',
            success: function (res) {
